...
// La prescripción ya viene incluida al abrir el detalle (openDetailMode),
// así que aquí solo la mostramos, sin pedirla aparte.
...
async function fetchPets() {
    try {
        const respuesta = await apiFetch('/mascotas', { method: 'GET' });
        // Adaptamos id_mascota -> id, para no tocar el resto del archivo
        petsData = (respuesta.mascotas || []).map(m => ({ ...m, id: m.id_mascota }));
    } catch (err) {
        console.error('No se pudo cargar el catálogo de mascotas:', err.message);
        petsContainer.innerHTML = `<p style="color: #666; font-size: 16px; grid-column: 1/-1; text-align: center;">No se pudo conectar con el servidor. Intenta de nuevo más tarde.</p>`;
        return;
    }
    renderGrid();
}
...
async function openDetailMode(id) {
    ...
    try {
        // Tu API regresa { ok, mascota, prescripcion } en un solo llamado
        const respuesta = await apiFetch(`/mascotas/${id}`, { method: 'GET' });
        if (respuesta.mascota) {
            pet = { ...pet, ...respuesta.mascota, id: respuesta.mascota.id_mascota, prescripcion: respuesta.prescripcion };
        }
    } catch (err) {
        console.warn('No se pudo cargar el detalle completo, se usa el del catálogo:', err.message);
    }
